...
void broadcast_signals(int signal){
    int floor;
    for(floor = OPE_PANE1; floor <= OPE_PANE5; floor++){
        send_signal(pid_list[OPE_PANE1], signal);
    }
}
...
void send_arrival_signals(int arrive_signal){
     if (call_signal < F2_ARRIVAL || call_signal > F5_ARRIVAL) {
        return FALSE;
    }
    ...
    for(floor = OPE_PANE1; floor <= OPE_PANE5; floor++) {
        if(floor != current_floor) {
            send_signal(pid_list[current_floor], arrive_signal);
        }
    }
