...
void sensor_change(int sigNo){	
	...
	} else if (sigNo == F2_ARRIVAL || sigNo == F3_ARRIVAL || sigNo == F4_ARRIVAL || sigNo == F5_ARRIVAL) {
		printf("arrivallll\n");
		i=sigNo-F1_ARRIVAL-1;	
		...
		switch(action){
			...
			case UP:
				if(pid_list[LIFT_POSITION] < 135) {
					...
					pid_list[LIFT_POSITION] += VELOCITY;					
					printf("body process run: increasing velocity: %d\n", pid_list[LIFT_POSITION]);
				}
				else {
					action=STAND;
					printf("body process run: standing\n");
				}
				break;
			default:
				break;
